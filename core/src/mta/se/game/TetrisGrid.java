...
public class TetrisGrid {
	 ...
	 */
	public static Array<Vector2> translateBlockCoords(Tetris block, Vector2 gridLocation) {
	...
	public Array<BlockDrawable> getGridBlocksToDraw() {
		...
		for(boolean[] row : grid) {
			...
			for(boolean block : row) {
				if (block) {
					blocks.add(new BlockDrawable(colNum, rowNum, Tetris.colors.GREEN));
				}
				colNum++;
			}
			rowNum++;
		...
		if (fallingBlock != null) {
			for(Vector2 block : translateBlockCoords(fallingBlock, fallingBlockLocation)) {
				if (block.y <= 19 && block.x <= 9){
					blocks.add(new BlockDrawable(Math.round(block.x), Math.round(block.y), Tetris.colors.BLUE));
				}
			}
		}
		return blocks;
